...
- **Inputs:**
  - **`id`** *(string)*:  
    Unique & arbitrary identifier to be used for retrieving this response later with `assert_response`.
  - **`url`** *(string)*:  
...
- **Inputs:**
  - **`id`** *(string)*:  
    Identifier of the HTTP response initially expected using `expect_response`.
