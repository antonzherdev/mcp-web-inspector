...
## Version 1.0.6
- **New Tool: `upload_file`**: Added a new tool to upload files to an `input[type='file']` element.
- **Enhanced Content Extraction**:
  - Significantly improved `get_visible_text` tool for more accurate text extraction.
  - Added `get_visible_html` tool to retrieve the full HTML content of the page.
  - **By default, all `<script>` tags are now removed from the HTML output for `get_visible_html` (unless `removeScripts: false` is set). Output is also truncated to a safe length (default 20,000 characters) to prevent issues with LLM clients.**
- **Improved Interactions**:
  - Updated `hover` functionality in the interaction tool.
- **Browser Support**:
...
- **Advanced Interaction**: Added new interaction tools for more complex scenarios 🔄
  ...
  - `press_key`: Press keyboard keys with optional element focus
- **Output Capabilities**: Added content export functionality 📄
  - `save_as_pdf`: Save the current page as a PDF file with customizable options
- **Content Extraction**: Added tools for retrieving page content 📝
  - `get_visible_text`: Extract all visible text content from the current page
  - `get_visible_html`: Get the complete HTML content of the current page
...
- **Multi-Browser Support**: Added support for Firefox and WebKit browsers in addition to Chromium 🌐
  - New `browserType` parameter for `navigate` tool allows specifying browser engine
...
- New Feature: Three new tools (Thanks to ***@VinceOPS***)
  ...
  ```
  - A new tool `custom_user_agent` to define a custom user agent.


## Version 0.3.1
- Fixed BROWSER_TOOLS as console_logs is not required (Thanks to https://github.com/kfern)
...
## Version 0.3.0
- Added support for `console_logs` to get the console logs from the browser. Following logs types
...
:::tip Usage Example
To invoke `console_logs` via MCP Playwright, use the following prompt:

```plaintext
Get the console log from the browser whenever you perform any action.
:::
- Added support for `close` to close the browser and release all resources.

:::tip Usage Example
To invoke `close` via MCP Playwright, use the following prompt:
